...
void main() {
  ...
  printf("Enter the priority of each process (lower number = higher priority):\n");
  for(i = 0; i < n; i++) {
    printf("Process %d: ", i + 1);
  ...
  int prev=-1;
  while(completed<n) {
    int mini = -1, maxpriority = INT_MAX;
    for(i=0;i<n;i++) {
      if(p[i].at<=time && p[i].done==0 && maxpriority>p[i].priority) {
